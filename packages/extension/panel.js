...
function fluxCopyBadge(btn) {
  var input = document.getElementById('badgeMdInput');
  if (!input) return;
  var text = input.value;
  // Textarea fallback — works in DevTools panels
  ...
  setTimeout(function() { btn.textContent = '📛 Copy Badge'; }, 1500);
}
...
function renderOverview() {
  ...
  // Badge
  var badgeColor = s.overall >= 90 ? 'brightgreen' : s.overall >= 70 ? 'blue' : s.overall >= 50 ? 'yellow' : 'red';
  var badgeUrl = 'https://img.shields.io/badge/FluxAPI_Score-' + s.overall + '%2F100-' + badgeColor;
  var badgeMd = '![FluxAPI Score](' + badgeUrl + ')';
  h += '<div style="text-align:center;padding:8px;margin-bottom:8px;border:1px dashed var(--border);border-radius:8px;background:rgba(124,92,252,0.04)">';
  h += '<img src="' + badgeUrl + '" style="height:18px;margin-bottom:4px" /><br>';
  h += '<input type="text" id="badgeMdInput" value="' + badgeMd.replace(/"/g, '&quot;') + '" style="position:absolute;left:-9999px" />';
  h += '<button id="copyBadgeBtn" style="background:none;border:1px solid var(--border);color:var(--accent);padding:2px 10px;border-radius:4px;font-size:10px;cursor:pointer;font-family:var(--font)">📛 Copy Badge</button>';
  h += '</div>';
  ...
  var copyBtn = document.getElementById('copyBadgeBtn');
  if (copyBtn) {
    copyBtn.addEventListener('click', function() { fluxCopyBadge(this); });
  }
}
...
function doExportHtml() {
  ...
  html += '.badge-section{text-align:center;padding:20px;margin-top:24px;border:1px dashed var(--border);border-radius:10px;background:var(--bg2)}';
  html += '.badge-code{display:inline-flex;align-items:center;gap:8px;background:var(--bg);border:1px solid var(--border);border-radius:6px;padding:4px 8px}';
  html += '.badge-code code{font-size:11px;color:var(--fg2);font-family:monospace;word-break:break-all}';
  html += '.badge-code button{background:#7c5cfc;color:white;border:none;padding:3px 10px;border-radius:4px;font-size:11px;cursor:pointer;white-space:nowrap}';
  ...
  html += '<div style="margin-bottom:8px"><img src="' + badgeUrl + '" alt="FluxAPI Score" style="height:22px" /></div>';
  html += '<div style="font-size:12px;color:var(--fg2);margin-bottom:6px">Add to your README:</div>';
  html += '<div class="badge-code"><code>' + esc(badgeMd) + '</code><button onclick="navigator.clipboard.writeText(\'' + badgeMd.replace(/'/g, "\\'") + '\');this.textContent=\'Copied!\';setTimeout(()=>this.textContent=\'Copy\',1500)">Copy</button></div>';
  html += '</div>';
